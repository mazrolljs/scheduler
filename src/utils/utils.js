...
export const generateNextEmpId = async (db) => {
  ...
  if (snapshot.empty) {
    console.log("No existing employees found. Starting with emp_001");
    return "emp_001";
  }
  ...
  if (!match) {
    // If the format is wrong, fallback to emp_001 or handle gracefully
...
export const fetchLocations = async (db) => {
